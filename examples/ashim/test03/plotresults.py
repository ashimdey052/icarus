...
def plot_cache_hits_vs_alpha(resultset, topology, cache_size, alpha_range, strategies, plotdir):
    ...
    desc['xlabel'] = u'Content distribution \u03B1' +  r'$ \alpha $'# u'/u03b1'
    desc['xparam'] = ('workload', 'alpha') #experiment['workload']['alpha'] = alpha  Path to the value of the x axis metric, e.g. ['workload', 'alpha']
    desc['xvals'] = alpha_range #Range of x values, e.g. [0.6, 0.7, 0.8, 0.9]
    
    desc['filter'] = {'topology': {'name': topology},
                      'cache_placement': {'network_cache': cache_size}} 
    desc['ymetrics'] = [('CACHE_HIT_RATIO', 'MEAN')] * len(strategies) #it is a 2-value list where the first value is the name of
                                                                       #the collector which measured the metric and the second value is the metric name. 
    desc['ycondnames'] = [('strategy', 'name')] * len(strategies)   #List of condition names specific to each line of the graph
...
def plot_cache_hits_vs_cache_size(resultset, topology, alpha, cache_size_range, strategies, plotdir):
    ...
    desc['ylabel'] = 'Cache hit ratio'
    desc['xscale'] = 'log'
    desc['xparam'] = ('cache_placement', 'network_cache') #experiment['cache_placement']['network_cache'] = network_cache
...
def plot_link_load_vs_alpha(resultset, topology, cache_size, alpha_range, strategies, plotdir):
    ...
    desc['xlabel'] = u'Content distribution \u03b1'+ r'$ \alpha $'
...
def plot_latency_vs_cache_size(resultset, topology, alpha, cache_size_range, strategies, plotdir):
    ...
    desc['errorbar'] = True
    desc['legend_loc'] = 'upper right'
    #desc['ymax'] = 120.0 #### added for test
    ...
    desc['bar_color'] = STRATEGY_BAR_COLOR ##added
    #desc['bar_hatch'] = STRATEGY_BAR_HATCH
    #desc['line_style'] = STRATEGY_STYLE
    desc['legend'] = STRATEGY_LEGEND
    desc['plotempty'] = PLOT_EMPTY_GRAPHS
    
    plot_bar_chart(resultset, desc, 'LATENCY_T=%s@A=%s.pdf'  #plot_lines chilo
               % (topology, alpha), plotdir)
