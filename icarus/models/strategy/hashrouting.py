...
class BaseHashrouting(Strategy):
    ...
    def authoritative_cache(self, content, cluster=None):
        ...
        h = hash(content)#two content must be same hash value
        if cluster is not None:
            return self.clusters[cluster][h % self.cluster_size[cluster]] # caches according to cluster
        return self.cache_assignment[h % self.n_cache_nodes]                # No cluster , overall hash
    ...
    def process_event(self, time, receiver, content, log):
        ...
        if self.controller.get_content(cache):
            ...
            if self.routing == 'SYMM':
                ...
                if cache in self.view.shortest_path(source, receiver):# if cache relies on the shortest path between source & receiver then cache or not
                    # Forward to cache
                    self.controller.forward_content_path(source, cache)
                    # Insert in cache
                    self.controller.put_content(cache)
            ...
            elif self.routing == 'MULTICAST':# cache the content not relative to node relies on shortest path or not
                if cache in self.view.shortest_path(source, receiver):
                    ...
                    for i in range(1, min([len(cache_path), len(recv_path)])): #Where the path devides?
                    ...
                    self.controller.forward_content_path(fork_node, cache,      # Not in the return path, so main_path=false
                                                         main_path=False)
                self.controller.put_content(cache)
            else:
...
class HashroutingEdge(BaseHashrouting):
    ...
    routed to the closest caching node and this caching node holds a copy of
    ...
    def process_event(self, time, receiver, content, log):#reqeust sent to proxy if miss ->cache if miss->source
        ...
        if proxy != cache:
            if self.controller.get_content_local_cache(proxy):
                self.controller.forward_content_hop(proxy, receiver)# only 1 hop dure 
                self.controller.end_session()
                return
            else:
                # Forward request to authoritative cache
        ...
        if self.routing == 'SYMM':
            ...
            for u, v in cache_branch:
                ...
                if v == cache:
                    ...
                    self.controller.put_content_local_cache(v)      # v's local cache
...
@register_strategy('ASHIM_HR_HYBRID_SM') #1st Till 28/02
...
class HashroutingOnPathLCD(BaseHashrouting):
    ...
    def __init__(self, view, controller, on_path_cache_ratio=0.2, max_stretch=0.2, **kwargs):
        ...
        if on_path_cache_ratio < 0 or on_path_cache_ratio > 1:
            raise ValueError('on_path_cache_ratio must be between 0 and 1')
        super(HashroutingOnPathLCD, self).__init__(view, controller)
        self.max_stretch = nx.diameter(view.topology()) * max_stretch
...
@register_strategy('HR_ON_PATH_CL4M')
class HashroutingOnPathCL4M(BaseHashrouting):
    """Hybrid hash-routing and on-path caching.
    ...
    def __init__(self, view, controller, on_path_cache_ratio=0.1, max_stretch=0.2,use_ego_betw=False, **kwargs):
        ...
        if on_path_cache_ratio < 0 or on_path_cache_ratio > 1:
            raise ValueError('on_path_cache_ratio must be between 0 and 1')
        super(HashroutingOnPathCL4M, self).__init__(view, controller)
        self.max_stretch = nx.diameter(view.topology()) * max_stretch
        self.controller.reserve_local_cache(on_path_cache_ratio)#by_default_minimum = 0.1
...
@register_strategy('HR_ON_PATH_MCD') #Move Copy Down
class HashroutingOnPathMCD(BaseHashrouting):
    """Hybrid hash-routing and on-path caching.
    ...
    def __init__(self, view, controller, on_path_cache_ratio=0.1, max_stretch=0.2, **kwargs):
        ...
        if on_path_cache_ratio < 0 or on_path_cache_ratio > 1:
            raise ValueError('on_path_cache_ratio must be between 0 and 1')
        super(HashroutingOnPathMCD, self).__init__(view, controller)
        self.max_stretch = nx.diameter(view.topology()) * max_stretch
        self.controller.reserve_local_cache(on_path_cache_ratio)#by_default_minimum = 0.1
